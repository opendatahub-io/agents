# Validate Function Tool Calling with GPT-OSS via Responses API (RHAIENG-2235)

## Objective

Manually test that GPT-OSS models can correctly invoke function tools through the Llama Stack Responses API.

## Test Results (01/14/2026)
...
4. **Function Tool**: Created a weather function calling agent using the Responses API with a `get_weather` tool that fetches real data
...
**Both GPT-OSS-20b and GPT-OSS-120b successfully invoke function tools through Llama Stack Responses API** using vLLM current main branch.
